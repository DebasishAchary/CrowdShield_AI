...
type SourceTab = 'camera' | 'network' | 'file';
...
export const UploadVideo: React.FC = () => {
  ...
  const handleCameraConnect = async () => {
    ...
    try {
      ...
      setActiveSource('0');
      showSuccess(response.message || 'Local camera connected.');
    } catch (error: any) {
      showError(
  ...
  const handleNetworkConnect = async () => {
    ...
    if (
      ...
      showError('Use a valid RTSP, HTTP, or HTTPS stream URL.');
      return;
    }
    ...
    try {
      const response = await crowdService.setVideoSource(source);
      setActiveSource(source);
      showSuccess(response.message || 'Network stream connected.');
    } catch (error: any) {
      showError(
        error?.response?.data?.detail ||
  ...
  const handleFileChoose = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file.size > maxSize) {
      showError('Video file must be 500MB or smaller.');
      return;
    }
  ...
  const handleFileUpload = async () => {
    ...
    try {
      const response = await crowdService.uploadVideoFile(selectedFile);
  ...
  const handleReset = () => {
    ...
    toast('Video source configuration reset.', {
      icon: '🔄',
      style: toastStyle,
    });
  };
  ...
  return (
    <div className="bg-[#1E293B] rounded-xl p-5 border border-slate-700/60 shadow-lg space-y-4">
      <div className="flex items-center justify-between border-b border-slate-700/60 pb-3 gap-3">
        <div>
          ...
          <p className="text-xs text-slate-400">
            Select a camera, network stream, or recorded video.
          </p>
        </div>
      ...
      {/* Source tabs */}
      <div className="grid grid-cols-3 gap-1 rounded-lg bg-slate-900/70 p-1 border border-slate-700">
        {tabs.map((tab) => {
          const Icon = tab.icon;
      ...
      {activeTab === 'camera' && (
        <div className="rounded-xl border border-slate-700 bg-slate-900/60 p-5">
          <div className="flex items-center gap-3 mb-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500">
                Use the server machine's default webcam (camera 0).
              </p>
            </div>
          </div>
          ...
          >
            ...
            {isConnecting ? 'Connecting...' : 'Connect Local Camera'}
          </button>
        </div>
      )}
        ...
        <div className="rounded-xl border border-slate-700 bg-slate-900/60 p-5">
          <div className="flex items-center gap-3 mb-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500">
                Connect an RTSP, HTTP, or HTTPS IP-camera/CCTV stream.
              </p>
            </div>
          </div>
          ...
          <p className="mt-2 text-[10px] text-slate-500 font-mono">
            Example: rtsp://camera-address/stream or
            http://camera-address:port/video
          </p>
        </div>
      )}
      ...
      {activeTab === 'file' && (
        <div className="space-y-3">
          ...
          >
            ...
            {isConnecting ? 'Uploading & Starting...' : 'Upload & Start Analysis'}
          </button>
        </div>
      )}
